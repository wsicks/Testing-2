...
const FEATURE_SEEDS: FeatureSeed[] = [
  ...
  { id: "complement_arb", name: "Complement Arbitrage", thesis: "When the YES and NO books of ONE market sum under $1 net of fees, buying both sides locks profit at resolution regardless of outcome — arithmetic, not forecasting. Exists only in moments of cross-book mispricing; capacity is top-of-book thin.", whyMissed: "Requires watching BOTH tokens' books simultaneously and pairing two legs fast; single-book tooling can never see it (one book implies yesAsk+noAsk = 1+spread by construction).", dataSources: ["polymarket_clob"], categoryScope: ["all"], status: "paper_testing", killCriteria: "no net-positive pair found over 30 days of scanning, or unwind rate > 20% (books reprice faster than we can pair)" },
